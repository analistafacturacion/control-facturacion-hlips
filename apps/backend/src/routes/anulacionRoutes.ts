...
// Endpoint para descargar archivo plano de anulaciones (template)
router.get('/archivo-plano-template', async (req, res) => {
    try {
        const ExcelJS = require('exceljs');
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet('Archivo Plano Anulaciones');

        // Configurar columnas
        worksheet.columns = [
            { header: 'Número Anulación', key: 'numeroAnulacion', width: 20 },
            ...
            { header: 'Total Anulado', key: 'totalAnulado', width: 15 }
        ];

        // Agregar algunas filas de ejemplo con formato
        worksheet.addRow({
            numeroAnulacion: 'Ejemplo: AN001',
            fecha: '2025-01-01',
            sede: 'Ejemplo: Sede Principal',
            usuario: 'usuario@ejemplo.com',
            motivo: 'Ejemplo de motivo',
            estado: 'Pendiente',
            factura: 'F001',
            aseguradora: 'EPS Ejemplo',
            paciente: 'Juan Pérez',
            tipoDocumento: 'CC',
            documento: '12345678',
            totalAnulado: 100000
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=Archivo_plano_anulaciones.xlsx');
        
        await workbook.xlsx.write(res);
        res.end();

    } catch (error) {
        console.error('Error generando archivo plano:', error);
        res.status(500).json({ error: 'Error al generar archivo plano' });
    }
});
