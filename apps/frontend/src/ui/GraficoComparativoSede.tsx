...
interface Props {
  data: { sede: string; año: number; mes: number; valor: number }[];
  sedes: string[];
  años: number[];
}
...
export const GraficoComparativoSede: React.FC<Props> = ({ data, sedes, años }) => {
  const [sede, setSede] = useState(sedes[0] || '');
  const [año, setAño] = useState(años[años.length-1] || new Date().getFullYear());

  // Generar datos para el gráfico: total facturado por sede en cada mes
  const datosGrafico = meses.map((mes, idx) => {
    const eventosMes = data.filter(d => d.sede === sede && d.año === año && d.mes === idx+1);
